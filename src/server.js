...
app.post('/wallets/:userId/withdrawals', async (req, res) => {
  ...
  try {
    ...
    if (!recipientCode) {
      ...
      setBankDetails(userId, bank.account_number, bank.bank_code);
      db
        .prepare('UPDATE bank_details SET recipient_code = ?, account_name = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?')
        .run(recipientCode, recipient.details?.account_name || null, userId);
    }
