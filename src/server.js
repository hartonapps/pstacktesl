...
app.post('/wallets/:userId/bank-details', async (req, res) => {
  const { userId } = req.params;
  const { accountNumber, bankCode, skipResolve = false } = req.body;
  ...
  try {
    ...
    return res.json({
      message: skipResolve
        ? 'Bank details saved without Paystack resolve.'
        : 'Bank details saved and resolved successfully.',
      resolvedAccountName: resolved?.account_name || null,
      bankId: resolved?.bank_id || null
    });
  } catch (error) {
    return res.status(400).json({
      error: 'Cannot resolve account with Paystack. Check account number/bank code or use simulation mode.',
...
app.get('/banks/resolve', async (req, res) => {
  ...
  if (!/^\d{10}$/.test(accountNumber) || !/^\d+$/.test(bankCode)) {
    return res.status(400).json({ error: 'Provide valid accountNumber (10 digits) and numeric bankCode.' });
  }
