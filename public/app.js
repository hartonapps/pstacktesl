...
$('resolveBank').onclick = async () => {
  try {
    const accountNumber = $('accountNumber').value.trim();
    const bankCode = $('bankCode').value.trim();
    const data = await api(`/banks/resolve?accountNumber=${encodeURIComponent(accountNumber)}&bankCode=${encodeURIComponent(bankCode)}`);
    $('resolveOutput').textContent = JSON.stringify(data, null, 2);
    setStatus(`Account resolved: ${data.accountName}`);
  } catch (e) {
    $('resolveOutput').textContent = '';
    setStatus(e.message, true);
  }
};
...
$('saveBank').onclick = async () => {
  try {
    const data = await api(`/wallets/${getUserId()}/bank-details`, 'POST', {
      accountNumber: $('accountNumber').value.trim(),
      bankCode: $('bankCode').value.trim(),
      skipResolve: $('skipResolve').checked
    });
    setStatus(data.resolvedAccountName ? `Bank saved: ${data.resolvedAccountName}` : 'Bank details saved.');
  } catch (e) {
    setStatus(e.message, true);
  }
};
